...
export class Utils {
  ...
  static isFutureFY(mnth) {
    ...
    if (mn >= 4) {
      // current calendar year 20 .. for example current month is 8
      if (mnth > mn) {
  ...
  static convertDateFromServerToSystem(input) {
    // input is always in the form dd/mm/yyyy
    if (input == undefined || input == "") {
      return new Date();
    }
  ...
  static convertDateFromServerToStrddmmyyyy(input) {
    // input is always in the form dd/mm/yyyy
    if (input == undefined || input == "") {
      return new Date();
    }
  ...
  static convertDBDateShort(input, splitChar): string {
    // dd/mm/yyyy hh:mm:ss
    if (input == undefined || input == "") {
  ...
  static convertDBDateShortyyyymmdd(input, splitChar): string {
    // yyyy-mm-dd format
    if (input == undefined || input == "") {
      return new Date().toString();
    }
    var items = input.split(splitChar);
    return (
      items[2].split(" ")[0] + "/" + items[1] + "/" + items[0].substr(2, 2)
    );
  }
  ...
  static encryptString1(str) {
    let encryptedData = "";
    if (str != null && str != "") {
      encryptedData = CryptoJS.AES.encrypt(str.trim(), "1234123412341234").toString();
    }
    return encryptedData;
  }
  ...
  static decryptString(str) {
    let decryptedData = "";
    if (str != null && str != "") {
      decryptedData = CryptoJS.AES.decrypt(str, "1234").toString(
        CryptoJS.enc.Utf8
      );
    }
  ...
  static decryptResponse(str) {
    const hash = CryptoJS.enc.Utf8.parse('Etherart1@#$%^&*');
    const bytes = CryptoJS.AES.decrypt(str, hash, {
  ...
  static getBidRange(bids: [any], user_id) {
    ...
    bids.forEach((bid) => {
      ...
      if (bid.bid_price < obj.low) obj.low = bid.bid_price;
      if (bid.bid_price > obj.high) obj.high = bid.bid_price;
      obj.cnt++;
    });
    return obj;
  }
  ...
  static getBTCRate(): number {
    if (this.btcRate == 0) {
      let rate = DataService.getDataFromLocal("btcRate");
      if (rate == null) this.btcRate = 225347.12;
      else this.btcRate = parseFloat(rate);
    }
    return this.btcRate;
  }

  static setBTCRate(val) {
    this.btcRate = val;
    DataService.setDataInLocal("btcRate", val);
  }

  static getCollectionForMystery() {
    let col_list = CollectionService.getCollectionListCached();
    if (col_list == null)
      return null
    if (col_list.length > 0)
      return col_list[0].collection_id
  }

  static getImageIDForMystery() {
    let col_list = CollectionService.getCollectionListCached();
    if (col_list != null && col_list.length > 0) {
      return col_list[0].image_id
    } else {
      return 1;
    }
  }

  static getCollectionForNFT(item) {
    let col_list = CollectionService.getCollectionListCached();
    let i = col_list.findIndex(
      (col) => col.collection_id == item.collection_id
    );
    if (i >= 0) {
      return col_list[i];
    }
