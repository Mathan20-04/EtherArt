...
export class AuthService extends BaseService {
  ...
  */
    ...
    return this.postCommon(this.authenticateUrl, params, false)
      .pipe(
        map(data => {
          ...
          this.postAuthOrSignup(data, json.email_mobile, json.password) ;
          return data;
        }),
        catchError(ex => {
  ...
  logOut() {
    ...
    DataService.deleteFromLocal(AppConstants.DATA);
  ...
  */
  ...
  private postAuthOrSignup(data , mobile, password) {
    ...
    DataService.setDataInLocal(AppConstants.DATA, data);

    sessionStorage.setItem(AppConstants.AUTH_KEY, data['token'])
    sessionStorage.setItem(AppConstants.USER_OBJECT_KEY, JSON.stringify(data['data']));
  }
  ...
  activate(params,mobile, pwd): Observable<any> {
    return this.postCommon(this.activateUrl, params, false)
      .pipe(
        map(data => {
          // console.log(this.mobile, this.pwd)
          this.postAuthOrSignup(data, mobile, pwd)
          return data;
        }),
        catchError(ex => {
          return throwError(ex);
