...
export class ApiService {
  ...
  private appendHeaders() {
    ...
    this.httpHeaders = new HttpHeaders({
      ...
      'x-client-id': '17286467ac1f874fe3f8f36de6468271',
      'x-client-secret': 'd4869568ac08c53350e92e70c239c7e6ed16628e'
    });
    console.log(this.httpHeaders)
  }

  private getAccessToken() {
    let token = DataService.getDataFromLocal(AppConstants.AUTH_KEY);
    if (token == null) {
      token = sessionStorage.getItem(AppConstants.AUTH_KEY)
    }
    return token
  }
   ...
   */
  public postData(url, paramsObject, enableCredentials = true, isCompleteUrl: boolean = false, respType: string = null, appendHdrs:boolean = false): Observable<any> {
    try {
      if (appendHdrs)
        this.appendHeaders()
      else
  ...
  uploadFileTrans(file: File, url: string,  paramsObject: any,jsonKey: string): any{
    try {
  ...
  private handleHttpException(exception: HttpErrorResponse) {
    switch (exception.status) {
      case 500:
        return new GeneralException(exception.error.response_message, ErrorCodes.HTTP_ERROR, ErrorMessages.INTERNAL_ERROR);
        break;
      case 401:
        return new GeneralException(exception.error.response_object, ErrorCodes.HTTP_ERROR, ErrorMessages.NO_DATA_ERROR);
        break;
      case 400 :
        return new GeneralException(exception.error.response_object, ErrorCodes.GENERAL_ERROR, exception.error);
        break;
      default:
        return new GeneralException(exception.error.response_object, ErrorCodes.HTTP_ERROR, ErrorMessages.INTERNAL_ERROR);
        break;
