...
export abstract class BaseService {
  ...
  * */
   ...
   * @returns list of actual model or json array
   */
  protected postCommon(url: string, paramsObject, enableCredentials: boolean, isCompleteUrl: boolean = false, responseType: string = null,appendHdr=false): Observable<any> {
    return this.apiService.postData(url, paramsObject, enableCredentials, isCompleteUrl, responseType,appendHdr)
      .pipe(
        map(response => {
          return response;
        }),
        catchError(ex => {
          // console.log("base service", ex)
          return throwError(ex);
        })
      );
  }
...
/**
   ...
   * @param jsonKey
   */
 uploadFileTrans(file: File, url: string, params: any, jsonKey : string ) {
  return this.apiService.uploadFileTrans(file, url, params,jsonKey)
    .pipe(
      map(response => {
        return response;
      }),
   ...
   * @param responseType
   */
  uploadFile(files: Array<File>, userID, objectType: string, documentType: string, transactionID: string, isCompleteUrl: boolean = false, responseType: string = null) {
    return this.apiService.uploadFile(this.fileUploadURL, files, userID, objectType, documentType, transactionID, false, isCompleteUrl, responseType)
      .pipe(
        map(response => {
          return response;
        }),
